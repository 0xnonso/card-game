...
import {
    ...
    PendingAction as GamePendingAction
...
contract WhotRuleset is IRuleset, SepoliaConfig {
    ...
    function resolveMove(
        ResolveMoveParams memory params
    ) public returns (Effect memory effect) {
        ...
        if (params.gameAction.eqs(GameAction.Play)) {
            ...
            if (params.card.pickTwo()) {
                ...
                uint8 nextTurn = params.playerStoreMap.getNextIndexFrom_RL(
                    params.currentPlayerIndex
                );
                ...
                effect.nextPlayerIndex = playerStoreMap.getNextIndexFrom_RL(
                    playerStoreMap.getNextIndexFrom_RL(
                        params.currentPlayerIndex
                    )
                ); // Set turn to 1 for hold on op
                ...
                (WhotCardStandardLibx8.CardShape wishShape) = abi.decode(
                    params.extraData,
                    (WhotCardStandardLibx8.CardShape)
                );
    ...
    ) public view returns (uint8 startIdx) {
    ...
    ) public pure returns (uint8 nextTurnIdx) {
